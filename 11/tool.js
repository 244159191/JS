...
//【注】绑定事件的兼容性写法
		...
		function funcMove(){
			...
			if(aUl_img.offsetLeft % 950 ==0){
			startMove(aUl_img, {left: aUl_img.offsetLeft - 950})};
			clearInterval(timer);
			timer = setInterval(function(){
				if(aUl_img.offsetLeft < -aUl_img.offsetWidth / 2){
					aUl_img.style.left =" -950px";
				}
				startMove(aUl_img, {left: aUl_img.offsetLeft - 950});
			},  3000);
		}
		function funcMove2(){
			...
			clearInterval(timer);
			timer = setInterval(function(){        
			 if(aUl_img.offsetLeft > -950){
					 aUl_img.style.left =  "-2850px";
					}
					startMove(aUl_img, {left: aUl_img.offsetLeft + 950});
				},  3000);
			}
